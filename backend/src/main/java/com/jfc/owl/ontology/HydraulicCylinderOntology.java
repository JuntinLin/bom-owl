 ...
 */
/**
 * 2025-06-20 Optimized singleton ontology class for hydraulic cylinder domain
 * knowledge. This class creates and manages domain-specific classes,
 * properties, and relationships for hydraulic cylinder components and their BOM
 * structures.
 */
@Component
public class HydraulicCylinderOntology {
			...
			synchronized (HydraulicCylinderOntology.class) {
				if (instance == null) {
					...
					try {
						...
						logger.error("Failed to initialize ontology at startup", e);
	...
	/**
	 * Cleanup when the bean is destroyed
	 */
	@PreDestroy
	public void cleanup() {
		lock.writeLock().lock();
		try {
			if (ontModel != null) {
				try {
					...
					logger.debug("Error closing ontology model: {}", e.getMessage());
				}
				ontModel = null;
			}
			initialized = false;
	...
	/**
	 * Check if the model is still valid (not closed)
	 */
	private boolean isModelValid() {
		...
		try {
			// Test if model is still open by trying a simple operation
			ontModel.size();
			return true;
		} catch (ClosedException e) {
			logger.debug("Model is closed");
			return false;
		} catch (Exception e) {
			logger.debug("Model validation failed: {}", e.getMessage());
			return false;
		}
	}

	/**
	 * Ensure the model is valid, reinitializing if necessary
	 */
	private void ensureModelValid() {
		// Fast path - check without lock
		if (initialized && isModelValid()) {
			return;
		}
		...
		try {
			...
			if (!initialized || !isModelValid()) {
				logger.info("Ontology model is invalid or not initialized, reinitializing...");
				initialized = false; // Force full reinitialization
				initializeHydraulicCylinderOntologyInternal();
			}
		} finally {
			lock.writeLock().unlock();
		}
	...
	/**
	 * Initialize the hydraulic cylinder ontology with specialized classes and
	 * properties
	 */
	...
	/**
	 * Internal initialization method (must be called with write lock held)
	 */
	private void initializeHydraulicCylinderOntologyInternal() {
		// Check if already initialized AND model is valid
		if (initialized && isModelValid()) {
			logger.debug("Hydraulic Cylinder Ontology already initialized and valid");
			return;
		}
		...
		if (ontModel != null) {
			try {
				...
				logger.debug("Error closing existing model: {}", e.getMessage());
			}
			ontModel = null;
		}

		try {
			// Create ontology model with lighter reasoning support to save memory
			...
			logger.info("Hydraulic Cylinder Ontology initialization completed successfully");
		} catch (Exception e) {
			...
			throw new RuntimeException("Failed to initialize ontology", e);
		}
	}
	...
	private void addCardinalityConstraints() {
		...
		// 一個液壓缸必須有恰好一個缸筒
		Restriction hasExactlyOneBarrel = ontModel.createCardinalityRestriction(null, properties.get("hasComponent"),
				1);
		// 添加限定類型
		...
		// 一個液壓缸必須有恰好一個活塞
		Restriction hasExactlyOnePiston = ontModel.createCardinalityRestriction(null, properties.get("hasComponent"),
				1);
		...
		// 一個液壓缸必須有恰好一個活塞桿
		Restriction hasExactlyOnePistonRod = ontModel.createCardinalityRestriction(null, properties.get("hasComponent"),
				1);
		...
		// 一個液壓缸必須有至少一個密封組件
		Restriction hasAtLeastOneSealing = ontModel.createMinCardinalityRestriction(null,
				properties.get("hasComponent"), 1);
		...
		// 一個液壓缸通常有兩個端蓋
		Restriction hasTwoEndCaps = ontModel.createCardinalityRestriction(null, properties.get("hasComponent"), 2);
	...
	private void addValueRestrictions() {
		...
		Restriction smallBoreRestriction = ontModel.createAllValuesFromRestriction(null, properties.get("bore"),
				ontModel.getProfile().DATARANGE());
		classes.get("SmallBoreCylinder").addSuperClass(smallBoreRestriction);

		// Medium bore cylinders: 50mm < bore <= 100mm
		Restriction mediumBoreRestriction = ontModel.createAllValuesFromRestriction(null, properties.get("bore"),
				ontModel.getProfile().DATARANGE());
		classes.get("MediumBoreCylinder").addSuperClass(mediumBoreRestriction);

		// Large bore cylinders: bore > 100mm
		Restriction largeBoreRestriction = ontModel.createAllValuesFromRestriction(null, properties.get("bore"),
				ontModel.getProfile().DATARANGE());
		classes.get("LargeBoreCylinder").addSuperClass(largeBoreRestriction);

		// High pressure cylinders: maxPressure > 210 bar
		Restriction highPressureRestriction = ontModel.createAllValuesFromRestriction(null,
				properties.get("maxPressure"), ontModel.getProfile().DATARANGE());
	...
	public Individual createHydraulicCylinderIndividual(String itemCode, Map<String, String> specifications) {
		...
		lock.readLock().lock();
		try {
			// Get the hydraulic cylinder class
			OntClass cylinderClass = classes.get("HydraulicCylinder");
			if (cylinderClass == null) {
				throw new IllegalStateException("HydraulicCylinder class not found in ontology");
			}

			// Create the individual
			Individual cylinder = cylinderClass.createIndividual(HC_URI + "Cylinder_" + itemCode);

			// Add basic properties
			OntProperty itemCodeProp = properties.get("itemCode");
			if (itemCodeProp != null) {
				cylinder.addProperty(itemCodeProp, itemCode);
			}

			// Add specifications
			if (specifications.containsKey("bore") && properties.containsKey("bore")) {
				cylinder.addProperty(properties.get("bore"), specifications.get("bore"));
			}
			if (specifications.containsKey("stroke") && properties.containsKey("stroke")) {
				cylinder.addProperty(properties.get("stroke"), specifications.get("stroke"));
			}
			if (specifications.containsKey("series") && properties.containsKey("series")) {
				cylinder.addProperty(properties.get("series"), specifications.get("series"));
			}
			if (specifications.containsKey("rodEndType") && properties.containsKey("rodEndType")) {
				cylinder.addProperty(properties.get("rodEndType"), specifications.get("rodEndType"));
			}
			if (specifications.containsKey("installationType") && properties.containsKey("installationType")) {
			...
			// Model was closed during operation, retry
			logger.warn("Model was closed during cylinder creation, retrying...");
			lock.readLock().unlock();
			ensureModelValid();
			return createHydraulicCylinderIndividual(itemCode, specifications);
		} catch (Exception e) {
			logger.error("Error creating hydraulic cylinder individual: {}", itemCode, e);
			throw new RuntimeException("Failed to create cylinder individual", e);
		} finally {
	...
	private List<ComponentSuggestion> generateBarrelSuggestions(Map<String, String> specs) {
		...
		if (bore != null && series != null) {
			...
			barrel.setDescription(
					String.format("Standard barrel for %smm bore, Series %s hydraulic cylinder", bore, series));
			...
			if (Integer.parseInt(bore) > 80) {
				...
				corrosionResistantBarrel
						.setName(String.format("Stainless Steel Barrel %smm - Series %s", bore, series));
				corrosionResistantBarrel
						.setDescription("Corrosion-resistant stainless steel barrel for harsh environments");
	...
	private String getInstallationDescription(String installationType) {
		...
		switch (installationType) {
		case "FA":
			return "Front Attachment";
		case "RA":
			return "Rear Attachment";
		case "TM":
			return "Trunnion Mount";
		default:
			return "Standard";
		}
	}
