...
public class ReasoningService {
    ...
    public List<Map<String, String>> getPredefinedQueries() {
     ...
     // New queries for hydraulic cylinder properties
        Map<String, String> query6 = new HashMap<>();
        query6.put("name", "Find Hydraulic Cylinders by Bore and Stroke");
        query6.put("description", "Find hydraulic cylinders with specific bore and stroke values");
        query6.put("query", "PREFIX j.0: <http://www.jfc.com/tiptop/ontology#>\n" +
                           "SELECT ?cylinder ?name ?bore ?stroke\n" +
                           "WHERE {\n" +
                           "  ?cylinder a j.0:MasterItem .\n" +
                           "  ?cylinder j.0:itemName ?name .\n" +
                           "  ?cylinder j.0:bore ?bore .\n" +
                           "  ?cylinder j.0:stroke ?stroke .\n" +
                           "  FILTER(CONTAINS(?bore, \"050\"))\n" + // Example filter for 50mm bore
                           "  FILTER(CONTAINS(?stroke, \"0146\"))\n" + // Example filter for 146mm stroke
                           "}");
        queries.add(query6);
        
        Map<String, String> query7 = new HashMap<>();
        query7.put("name", "Count Hydraulic Cylinders by Series");
        query7.put("description", "Count the number of hydraulic cylinders by series");
        query7.put("query", "PREFIX j.0: <http://www.jfc.com/tiptop/ontology#>\n" +
                           "SELECT ?series (COUNT(?cylinder) as ?count)\n" +
                           "WHERE {\n" +
                           "  ?cylinder a j.0:MasterItem .\n" +
                           "  ?cylinder j.0:series ?series .\n" +
                           "}\n" +
                           "GROUP BY ?series\n" +
                           "ORDER BY DESC(?count)");
        queries.add(query7);
        
        Map<String, String> query8 = new HashMap<>();
        query8.put("name", "Find Cylinders by Installation Type");
        query8.put("description", "Find cylinders with a specific installation type");
        query8.put("query", "PREFIX j.0: <http://www.jfc.com/tiptop/ontology#>\n" +
                           "SELECT ?cylinder ?name ?installation\n" +
                           "WHERE {\n" +
                           "  ?cylinder a j.0:MasterItem .\n" +
                           "  ?cylinder j.0:itemName ?name .\n" +
                           "  ?cylinder j.0:installation ?installation .\n" +
                           "  FILTER(?installation = \"FA\")\n" + // Example: FA installation type
                           "}");
        queries.add(query8);
        
        Map<String, String> query9 = new HashMap<>();
        query9.put("name", "Find All Hydraulic Cylinder Properties");
        query9.put("description", "Show all properties of hydraulic cylinders for the current master item");
        query9.put("query", "PREFIX j.0: <http://www.jfc.com/tiptop/ontology#>\n" +
                           "SELECT ?property ?value\n" +
                           "WHERE {\n" +
                           "  ?master j.0:itemCode \"COMPONENT_CODE\" .\n" + // Will be replaced with actual master code
                           "  ?master ?property ?value .\n" +
                           "  FILTER(?property IN (\n" +
                           "    j.0:bore, j.0:stroke, j.0:rodEndType, \n" +
                           "    j.0:series, j.0:type, j.0:installation, j.0:shaftEndJoin\n" +
                           "  ))\n" +
                           "}");
        queries.add(query9);
        
        Map<String, String> query10 = new HashMap<>();
        query10.put("name", "Search by Rod End Type");
        query10.put("description", "Find cylinders with a specific rod end type");
        query10.put("query", "PREFIX j.0: <http://www.jfc.com/tiptop/ontology#>\n" +
                           "SELECT ?cylinder ?itemCode ?rodEndType\n" +
                           "WHERE {\n" +
                           "  ?cylinder a j.0:MasterItem .\n" +
                           "  ?cylinder j.0:itemCode ?itemCode .\n" +
                           "  ?cylinder j.0:rodEndType ?rodEndType .\n" +
                           "  FILTER(?rodEndType = \"Y\")\n" + // Example: Y rod end type
                           "}");
        queries.add(query10);
    ...
    public List<Map<String, String>> getExampleRules() {
     ...
     // New custom rules for hydraulic cylinder properties
        Map<String, String> rule4 = new HashMap<>();
        rule4.put("name", "Large Bore Cylinder Rule");
        rule4.put("description", "Identifies hydraulic cylinders with a bore size of 100mm or larger");
        rule4.put("rule", "[LargeBoreCylinderRule: (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#MasterItem>) " +
                         "(?cylinder <http://www.jfc.com/tiptop/ontology#bore> ?bore) " +
                         "regex(?bore, \"^(1[0-9][0-9]|[2-9][0-9][0-9])$\") " +
                         "-> (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#LargeBoreCylinder>)]");
        rules.add(rule4);
        
        Map<String, String> rule5 = new HashMap<>();
        rule5.put("name", "Long Stroke Cylinder Rule");
        rule5.put("description", "Identifies hydraulic cylinders with a stroke length of 200mm or longer");
        rule5.put("rule", "[LongStrokeCylinderRule: (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#MasterItem>) " +
                         "(?cylinder <http://www.jfc.com/tiptop/ontology#stroke> ?stroke) " +
                         "regex(?stroke, \"^(2[0-9][0-9][0-9][0-9]|[3-9][0-9][0-9][0-9][0-9])$\") " +
                         "-> (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#LongStrokeCylinder>)]");
        rules.add(rule5);
        
        Map<String, String> rule6 = new HashMap<>();
        rule6.put("name", "Y-Type Rod End Cylinder Rule");
        rule6.put("description", "Identifies cylinders with a Y-type rod end");
        rule6.put("rule", "[YTypeRodEndRule: (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#MasterItem>) " +
                         "(?cylinder <http://www.jfc.com/tiptop/ontology#rodEndType> \"Y\") " +
                         "-> (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#YTypeRodEndCylinder>)]");
        rules.add(rule6);
        
        Map<String, String> rule7 = new HashMap<>();
        rule7.put("name", "FA Installation Type Rule");
        rule7.put("description", "Identifies cylinders with FA installation type");
        rule7.put("rule", "[FAInstallationRule: (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#MasterItem>) " +
                         "(?cylinder <http://www.jfc.com/tiptop/ontology#installation> \"FA\") " +
                         "-> (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#FAInstallationCylinder>)]");
        rules.add(rule7);
        
        Map<String, String> rule8 = new HashMap<>();
        rule8.put("name", "Complex Configuration Rule");
        rule8.put("description", "Identifies cylinders with large bore, long stroke, and Y-type rod end");
        rule8.put("rule", "[ComplexConfigurationRule: (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#MasterItem>) " +
                         "(?cylinder <http://www.jfc.com/tiptop/ontology#bore> ?bore) " +
                         "(?cylinder <http://www.jfc.com/tiptop/ontology#stroke> ?stroke) " +
                         "(?cylinder <http://www.jfc.com/tiptop/ontology#rodEndType> \"Y\") " +
                         "regex(?bore, \"^(1[0-9][0-9]|[2-9][0-9][0-9])$\") " +
                         "regex(?stroke, \"^(2[0-9][0-9][0-9][0-9]|[3-9][0-9][0-9][0-9][0-9])$\") " +
                         "-> (?cylinder rdf:type <http://www.jfc.com/tiptop/ontology#ComplexConfigurationCylinder>)]");
        rules.add(rule8);
